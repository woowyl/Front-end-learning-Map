...
代码中有两个对象`person`和`programmer`两个对象，我需要的效果是，让`person`对象上的属性合并到`programmegir`对象上。(虽然看起来这不想是继承，好像用merge描述更准确一些)。

得到`programmer.origin == china`有两种方式，第一种就是直接让programmer.origin = china, 另一种方式是将被继承的对象放到继承者的原型链上。
...
这种方式比较简单粗暴，就是讲想要继承的对象放到继承者的原型链上。
