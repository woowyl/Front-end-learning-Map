...
- ### `$ git revert`
    ...
    如果你的代码已经被push到远程，或者你想撤销某一次提交，这时候回退就需要使用revert,和reset不同的是，它不会舍弃任何log记录，而是会**添加**一个新的commit 去记录某一次舍弃的操作，reset可能会修改多条提及，而revert是针对一条的撤销。

- ### `$git revert` 与 `$git reset`的区别

    是要撤回 **到** 某次提交，还是撤回某次提交？如果是撤回到某次提交使用`git reset` 如果是撤回某次提交，不对其他提交产生影响，使用`git revert`。
    `git reset`不会留下这次撤销操作。  
    `git revert`会显示一条撤回记录。
...
- ### `$ git merge [--no-ff <-m> 'mergelog']  <要合并的分支名>`
    ...
    没有冲突的情况下：
    ![ff](./image/fast-forward.png)
    有冲突的情况下，使用fast-forward仍然会保留分支上的信息
    ![noff](./image/log_squence.png)
...
- ### git fetch
    ...
    ![ff](./image/fetch_pull.png)  
    ![ff](./image/fetch_merge.png)  
...
- ### 2.4 origin
    ...
    去给仓库添加一个关联的远程.  
    可以通过`git remote -v`去查看现在已经添加的远程。
